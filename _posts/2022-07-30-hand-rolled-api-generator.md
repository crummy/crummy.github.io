...
* Java interfaces describe each "service" and its endpoints
...
almost as easy as native calls. Read on for details on how it works.

# Service definitions

This is the easy part. Our services look something like this:
...
# Backend implementation for our services
...
```
POST /services/UsersService/getUser
{
    'userId': 1001
}
```
...
public static void main(){
    ...
    // UsersService is the interface that defines the endpoints.
    // UsersApi is the backend implementation of UsersService.
    // We repeat the below for every service we want to expose.
    expose(app,UsersService.class,new UsersApi())
    app.start()
}

private <T> void expose(Javalin app, Class<T> service, T implementation) {
    String serviceName = service.getSimpleName();
    for (Method method : service.getMethods()) {
        // handle calls to, for example, POST /services/UsersService/getUser
        app.post("/services/" + serviceName + "/" + method.getName(), (ctx) -> {
            ...
            try {
                method.invoke(implementation, args.toArray());
            } catch (Exception e) {
...
That's pretty much it. For every service, then every method, expose an endpoint that
...
The code here works a little like the above again - given an interface like UsersService,
...
```
String toTypescript(Class... service) {
    for each service:
        typescript += "class ${service.getSimpleName()} {"
        for each method:
            ...
            typescript += "return fetch('/services/${service}/${method}', {"
...
Hmm... is that more readable? Well, here's what you might expect to see in the
generated Typescript, as a demonstration:

```typescript
class UsersService {
    getUser(userId: number): UserDto {
        return fetch('/services/UsersService/getUser', {
            method: 'POST',
...
We store the above file in `target/ts/services.ts`, and generate this file using the
...
Now you're almost ready to call `new UsersService().getUser(1001)` - we're just missing the
...
* A `services.ts` file with classes ready to query each of those endpoints
...
2. I told `typescript-generator` to convert Instants to numbers with:
```
<customTypeMappings>
